    ...
    if (token && userToEdit) {
      ...
      cy.apiRequest('GET', `/users/${userToEdit.email}`).then((getResponse) => {
        if (getResponse.status === 200) {
          ...
          const nombreUsuario = datosActuales.nombre_usuario
            || datosActuales.nombreUsuario;

          if (!nombreUsuario) {
            cy.log('No se pudo extraer nombre_usuario de la respuesta - test omitido');
            cy.log(`Estructura recibida: ${JSON.stringify(Object.keys(datosActuales))}`);
            return;
          }
